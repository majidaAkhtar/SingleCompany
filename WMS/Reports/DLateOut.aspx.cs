...
{
    public partial class DLateOut : System.Web.UI.Page
    {
        string PathString = "";
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                DataTable dt = qb.GetValuesfromDB("select * from ViewLateOut " + query + " and AttDate = '" + date.Date.Year.ToString() + "-" + date.Date.Month.ToString() + "-" + date.Date.Day.ToString() + "'" + " and StatusLO=1 ");
                List<ViewLateOut> _View = dt.ToList<ViewLateOut>();
        ...
        protected void btnGenerateReport_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt = qb.GetValuesfromDB("select * from ViewLateOut " + query + " and (AttDate >= " + _dateFrom + " and AttDate <= " + _dateTo + " )" + " and StatusLO=1 ");
            List<ViewLateOut> _ViewList = dt.ToList<ViewLateOut>();
        ...
        private void LoadReport(string path, List<ViewLateOut> _Employee)
        {
            string DateToFor = "";
            ...
            string _Header = "Late Out Report";
            this.ReportViewer1.LocalReport.DisplayName = "LateOut Report";
            ReportViewer1.ProcessingMode = ProcessingMode.Local;
            ReportViewer1.LocalReport.ReportPath = Server.MapPath(path);
            System.Security.PermissionSet sec = new System.Security.PermissionSet(System.Security.Permissions.PermissionState.Unrestricted);
            ReportViewer1.LocalReport.SetBasePermissionsForSandboxAppDomain(sec);
            IEnumerable<ViewLateOut> ie;
